    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-primary clearfix">
                <div class="box-body">
                    {!! Form::model($user, ['route' => ['admin.user.update', $user->id], 'enctype' => 'multipart/form-data', 'method' => 'put']) !!}
                                ...
                                @foreach($questions as $question)
                                    <div class="form-group clearfix">
                                        <div><label class="control-label"
                                                    for="question">{{ $question->content }}</label></div>
                                        <div>
                                            <label class="control-label"
                                                   for="question">{{ $question->answer_a }}</label>
                                            {!! Form::radio('question['.$question->id.']', 'a') !!}
                                            <label class="control-label"
                                                   for="question">{{ $question->answer_b }}</label>
